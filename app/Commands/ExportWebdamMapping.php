...
class ExportWebdamMapping extends Command
{
     ...
     * @var string
     */
    protected $signature = 'export:webdam-mapping {--token= : Widen Authentication token} {--f|filename= : Output filename.}';
    ...
    public function handle()
    {
        ...
        $this->task('Generate Webdam ID list', function() {
            Process::fromShellCommandline('drush sqlq "select field_acquiadam_asset_id_value from media__field_acquiadam_asset_id" > webdam_ids.csv')->run();
        });
        $rows = SimpleExcelReader::create('webdam_ids.csv')->getRows();
